...
class Bottleneck(nn.Module):
  ...
  def __init__(self, in_chans, out_chans, stride=1, atrous=1, downsample=None):
    ...
    self.conv2 = nn.Conv2d(
      out_chans, out_chans, kernel_size=3, stride=stride,
      padding=1 * atrous, dilation=atrous, bias=False
    )
    self.bn2 = nn.BatchNorm2d(out_chans)
    self.conv3 = nn.Conv2d(
      out_chans, out_chans * self.expansion, kernel_size=1, bias=False
    )
...
class ResNetAtrous(nn.Module):
  ...
  def _make_layer(block, in_chans, out_chans, blocks, stride=1, atrous=None):
    ...
    layers = [
      block(in_chans, out_chans, stride=stride, atrous=atrous[0], downsample=downsample)
    ]
...
def resnet50_atrous(pretrained=True, os=16, **kwargs):
  ...
  model = ResNetAtrous(Bottleneck, [3, 4, 6, 3], atrous=[1, 2, 1], os=os, **kwargs)
  if pretrained:
...
def resnet101_atrous(pretrained=True, os=16, **kwargs):
  ...
  model = ResNetAtrous(Bottleneck, [3, 4, 23, 3], atrous=[1, 2, 1], os=os, **kwargs)
  if pretrained:
